...
data STm info =
  ...
  | SPrint info String (STm info)
  | SUnPrint info String
  | SBinaryOp info BinaryOp (STm info) (STm info)
  | SFix info Name STy [([Name], STy)] (STm info)
  | SIfZ info (STm info) (STm info) (STm info)
  | SLet info Bool Name [([Name], STy)] STy (STm info) (STm info)
  deriving Show

type STerm = STm Pos       -- ^ 'STm' para términos 'azucarados', guarda posición
