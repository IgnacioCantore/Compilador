...
-- Versión modificada de `subst` para la optimización Constant Propagation,
-- ya que trabaja con términos que pueden no ser LC.
subst' :: Term -> Term -> Term
subst' n = varChanger (\_ p n -> V p (Free n)) bnd
   where bnd depth p i
             | i <  depth = V p (Bound i)
             | i == depth = n
