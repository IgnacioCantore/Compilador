...
printCharFD4 :: MonadFD4 m => Char -> m ()
printCharFD4 = liftIO . putChar
...
addSTy :: MonadFD4 m => Name -> Ty -> m ()
addSTy n ty = do
  styEnv <- gets styEnv
  let styEnv' = filter (\(nm,_) -> n /= nm) styEnv
  modify (\s -> s { styEnv = (n,ty) : styEnv' })
...
getOptimize :: MonadFD4 m => m Bool
getOptimize = gets optimize

getIters :: MonadFD4 m => m Int
getIters = gets iters
...
runFD4' b c =  runExceptT $ runStateT c $ initialEnv { optimize = b }
